...
// BadgerCache contains options to connect to a badger database
...
// BadgerConn is a connection to a badger database
...
type BadgerStats map[string]interface{}
...
func NewCache(defaultTimeout time.Duration, opts *badger.Options, gcOpts *GarbageCollectionOptions) (*BadgerCache, error) {
	...
	return &BadgerCache{opts: opts, gcOpts: gcOpts, TTL: defaultTimeout}, nil
}

// Open opens a new connection to Badger
func (c BadgerCache) Open(name string) (*BadgerConn, error) {
	c.opts.Dir = name
	...
	return &BadgerConn{TTL: c.TTL, ticker: ticker, db: db}, nil
}
...
func (c *BadgerConn) Write(k, v []byte) error {
	return c.WriteTTL(k, v, c.TTL)
}

// WriteTTL writes data to the cache with an explicit TTL
// a TTL of 0 does not expire keys
func (c *BadgerConn) WriteTTL(k, v []byte, ttl time.Duration) error {
...
func (c *BadgerConn) Read(k []byte) ([]byte, error) {
	ret := []byte{}
...
func (c *BadgerConn) Stats() map[string]interface{} {
	lsm, vlog := c.db.Size()
	return BadgerStats{
		"LSMSize":  lsm,
		"VLogSize": vlog,
...
func (c *BadgerConn) Backup(w io.Writer, since uint64) (upto uint64, err error) {
...
func (c *BadgerConn) Load(r io.Reader) error {
	return c.db.Load(r)
}
